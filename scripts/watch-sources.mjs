...
for (const m of spotsSrc.matchAll(/id: '([\w]+)'[\s\S]{0,700}?venues: \[([^\]]+)\]/g)) {
  for (const v of m[2].matchAll(/['"]([^'"]{3,})['"]/g)) {
    VENUES.push({ id: m[1], venue: v[1].replace(/\s*\([^)]*\)\s*/g, '').trim() })
  }
}
...
for (const f of FEEDS) {
  try {
    ...
    health.push(`${f.name}: ${items.length} items`)
    for (const it of items) {
      ...
      // an event needs a name for itself, or to be the weekly guide
      if (!flagship && !strong) continue
      const hits = (strong ? 3 : 0) + (weak ? 1 : 0) + (dated ? 2 : 0) + (flagship ? 12 : 0)

      // does it name somewhere we already map?
      const spotHit = SPOTS.find((s) => new RegExp(`\\b${s.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(hay))
      const venueHit = VENUES.find((v) => v.venue.length > 4 && new RegExp(`\\b${v.venue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(hay))
...
found.sort((a, b) => b.score - a.score || (b.date > a.date ? 1 : -1))
